...
def getItemsAttributesAndIcons(url):
    """Descarga de dota2 gamepedia los items = {'icon','atributos'}"""
    dictItems = {}
    try:
...
if __name__ == "__main__":
    items = getItemsAttributesAndIcons('https://dota2.gamepedia.com')
    try:
        import cPickle as pickle
    except ImportError:  # python 3.x
        import pickle

    with open('itemsAttrIcons.p', 'wb') as fp:
        pickle.dump(items, fp, protocol=pickle.HIGHEST_PROTOCOL)
